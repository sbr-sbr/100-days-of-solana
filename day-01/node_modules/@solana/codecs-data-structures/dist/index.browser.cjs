...
function buildFromEntries(entries) {
  return {
    build() {
      ...
      const decoder = fixedSize === null ? codecsCore.createDecoder({ read }) : codecsCore.createDecoder({ fixedSize, read });
      return decoder;
    },
...
function getPatternMatchEncoder(patterns) {
  return getUnionEncoder(
    ...
    (value) => {
      ...
      if (index === -1) {
        throw new errors.SolanaError(errors.SOLANA_ERROR__CODECS__INVALID_PATTERN_MATCH_VALUE);
      }
      return index;
    }
  );
...
function getPatternMatchDecoder(patterns) {
  return getUnionDecoder(
    ...
    (value) => {
      const index = patterns.findIndex(([predicate]) => predicate(value));
      if (index === -1) {
        throw new errors.SolanaError(errors.SOLANA_ERROR__CODECS__INVALID_PATTERN_MATCH_BYTES, {
          bytes: value
        });
      }
      return index;
