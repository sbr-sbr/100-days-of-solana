...
function assertValidMaxInstructionsPerTransaction(maxInstructions) {
  if (maxInstructions !== void 0 && (maxInstructions <= 0 || !Number.isInteger(maxInstructions) || maxInstructions > TRANSACTION_INSTRUCTION_LIMIT)) {
    throw new errors.SolanaError(errors.SOLANA_ERROR__INSTRUCTION_PLANS__INVALID_MAX_INSTRUCTIONS_PER_TRANSACTION, {
      maxInstructions,
...
function assertMaxInstructionsPerTransaction(numInstructions, maxInstructions) {
  if (numInstructions > maxInstructions) {
    throw new errors.SolanaError(errors.SOLANA_ERROR__INSTRUCTION_PLANS__MAX_INSTRUCTIONS_PER_TRANSACTION_EXCEEDED, {
      maxInstructions,
      numInstructions
    });
  }
      ...
      return Object.freeze({
        ...
        packMessageToCapacity: (message, config) => {
          ...
          for (let index = instructionIndex; index < instructions.length; index++) {
            ...
            const nextMessage = transactionMessages.appendTransactionMessageInstruction(instructions[index], message);
            const messageSize = transactions.getTransactionMessageSize(nextMessage);
            if (messageSize > transactions.getTransactionMessageSizeLimit(nextMessage)) {
              if (index === instructionIndex) {
                throw new errors.SolanaError(
                  errors.SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_CANNOT_ACCOMMODATE_PLAN,
                  {
                    ...
                    numFreeBytes: transactions.getTransactionMessageSizeLimit(nextMessage) - originalMessageSize
                  }
                );
              }
              instructionIndex = index;
...
function createTransactionPlanner(config) {
  return async (instructionPlan, { abortSignal, maxInstructionsPerTransaction } = {}) => {
    const resolvedMaxInstructionsPerTransaction = maxInstructionsPerTransaction ?? config.maxInstructionsPerTransaction;
